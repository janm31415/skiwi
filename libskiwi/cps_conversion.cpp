...
//both algorithms seem to be equally fast
  ...
  struct is_simple_helper
    ...
    void operator()(PrimitiveCall& p)
      {
      is_simple = true;
      for (auto& arg : p.arguments)
        {
        is_simple_helper ish;
        std::visit(ish, arg);
        is_simple &= ish.is_simple;
        if (!is_simple)
          break;
        }
      }

    void operator()(ForeignCall& p)
      {
      is_simple = true;
      for (auto& arg : p.arguments)
        {
    ...
    void operator()(Set& s)
      {
      is_simple_helper ish;
      std::visit(ish, s.value.front());
      is_simple = ish.is_simple;
      }

    void operator()(Let& l)
      {
      is_simple_helper ish1;
      std::visit(ish1, l.body.front());
      is_simple = ish1.is_simple;
      if (!is_simple)
        return;
      for (auto& arg : l.bindings)
        {
        is_simple_helper ish;
        std::visit(ish, arg.second);
        is_simple &= ish.is_simple;
        if (!is_simple)
          break;
        }
  ...
  bool is_simple(const Expression& e)
    {
    /*
    From Essentials of Programming Languages, Daniel P. Friedman and Mitchell Wand, p.208
    */
    ...
    if (std::holds_alternative<Set>(e))
      {
      return is_simple(std::get<Set>(e).value.front());
      }
    ...
    if (std::holds_alternative<Let>(e))
      ...
      for (const auto& arg : l.bindings)
        if (!is_simple(arg.second))
          return false;
      return is_simple(l.body.front());
      }
